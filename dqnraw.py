...
NUMBER_OF_EPISODES = 1000 # TODO: save and restore model with infinite episodes
EXPLORATION_RATE = 1
...
class ConvolutionalNeuralNetwork:
    ...
    @tf.function
    def predict(self, inputs): # 4D input for CNN: (batch_size, height, width, depth) 
...
class Agent:
    ...
    def replay(self):  # Train neural net with experiences, maybe use Experience: (state, action, reward, next_state, is_done) 
        ...
        for i, (state, action, reward, next_state, is_done) in enumerate(memory_batch):
            ...
            state_target[action] = reward if is_done else reward + DISCOUNT_FACTOR * tf.reduce_max(next_state_predictions[i])  # TODO: np.amax use tf.reduce_max
            inputs[i] = state
